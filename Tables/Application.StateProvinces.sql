...
GO
CREATE CLUSTERED INDEX [ix_StateProvinces_Archive] ON [Application].[StateProvinces_Archive] ([ValidTo], [ValidFrom]) WITH (DATA_COMPRESSION = PAGE) ON [USERDATA]
GO
...
CONSTRAINT [PK_Application_StateProvinces] PRIMARY KEY CLUSTERED  ([StateProvinceID]) ON [USERDATA]
) ON [USERDATA]
WITH
(
...
GO
CREATE NONCLUSTERED INDEX [FK_Application_StateProvinces_CountryID] ON [Application].[StateProvinces] ([CountryID]) ON [USERDATA]
GO
CREATE NONCLUSTERED INDEX [IX_Application_StateProvinces_SalesTerritory] ON [Application].[StateProvinces] ([SalesTerritory]) ON [USERDATA]
GO
ALTER TABLE [Application].[StateProvinces] ADD CONSTRAINT [UQ_Application_StateProvinces_StateProvinceName] UNIQUE NONCLUSTERED  ([StateProvinceName]) ON [USERDATA]
GO
